...
#pragma HLS LOOP_TRIPCOUNT min=5 max=5 avg=5
		...
		{
#pragma HLS LOOP_TRIPCOUNT min=5 max=5 avg=5
			for(int input_ch_idx=0;input_ch_idx<MAX_KERNEL_NUM/8;input_ch_idx++)
			{
//#pragma HLS ALLOCATION instances=logistic_activate limit=2 function
#pragma HLS PIPELINE
				...
				if(activate_en.bit(8*input_ch_idx))
				...
				if(activate_en.bit(8*input_ch_idx+1))
				...
				if(activate_en.bit(8*input_ch_idx+2))
				...
				if(activate_en.bit(8*input_ch_idx+3))
