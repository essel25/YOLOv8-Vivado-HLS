...
#pragma HLS STREAM variable=out_stream_group depth=2 dim=1
	...
	line_buff_type line_buff_group_0[MAX_KERNEL_NUM/8];
	line_buff_type line_buff_group_1[MAX_KERNEL_NUM/8];
	line_buff_type line_buff_group_2[MAX_KERNEL_NUM/8];
	line_buff_type line_buff_group_3[MAX_KERNEL_NUM/8];
...
#pragma HLS LOOP_TRIPCOUNT min=3 max=3
				...
				local_mem_group[k][i].data[8*j] = curr_input.data.sub_data_0;
				if(j!=(MAX_KERNEL_DIM*MAX_KERNEL_DIM+3)/8-1)
				{
					local_mem_group[k][i].data[8*j+1] = curr_input.data.sub_data_1;
					local_mem_group[k][i].data[8*j+2] = curr_input.data.sub_data_2;
					local_mem_group[k][i].data[8*j+3] = curr_input.data.sub_data_3;
				}
			}
...
//	}
	...
	{
#pragma HLS LOOP_TRIPCOUNT min=643 max=643
				...
				if(row_idx != input_h)
				{
					...
					if((row_idx>MAX_KERNEL_DIM-2)&&(col_idx>MAX_KERNEL_DIM-2))
					{
						...
						for(int kernel_idx=0; kernel_idx<MAX_KERNEL_NUM; kernel_idx++)
						{
							...
							//core of conv, macc
							sub0_val_output = window_macc(kernel_window_0,local_mem_group[kernel_idx][8*input_ch_idx]);
							sub1_val_output = window_macc(kernel_window_1,local_mem_group[kernel_idx][8*input_ch_idx+1]);
							sub2_val_output = window_macc(kernel_window_2,local_mem_group[kernel_idx][8*input_ch_idx+2]);
							if(input_ch==3)
							{
								sub3_val_output = 0;
							}
							...
							{
								sub3_val_output = window_macc(kernel_window_3,local_mem_group[kernel_idx][8*input_ch_idx+3]);
							}
...
void out_stream_merge(yolo_inter_stream out_stream_group[MAX_KERNEL_NUM], yolo_quad_stream &outStream, int input_ch_idx,quad_fp_side_channel curr_input,ap_uint<1> last,int output_ch,int fold_output_ch )
{
	...
	for(int i=0; i<STREAM_TX_SIZE; i++)
	{
		...
		if(4*kernel_idx<MAX_KERNEL_NUM)
			if(!(out_stream_group[8*kernel_idx].empty()))
			{
				quad_fp_side_channel curr_output;

				curr_output.data.sub_data_0 = out_stream_group[8*kernel_idx].read();
				curr_output.data.sub_data_1 = out_stream_group[8*kernel_idx+1].read();
				curr_output.data.sub_data_2 = out_stream_group[8*kernel_idx+2].read();


				if(!(out_stream_group[8*kernel_idx+3].empty()))
				{
					curr_output.data.sub_data_3 = out_stream_group[8*kernel_idx+3].read();
				}
				else
				{
