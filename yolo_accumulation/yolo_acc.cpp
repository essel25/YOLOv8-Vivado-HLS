...
#pragma HLS ARRAY_PARTITION variable=kernel_bias_fp cyclic factor=2 dim=1
...
#pragma HLS PIPELINE
		...
		kernel_bias_fp[8*i] = curr_input.data.sub_data_0;
		kernel_bias_fp[8*i+1] = curr_input.data.sub_data_1;
		kernel_bias_fp[8*i+2] = curr_input.data.sub_data_2;
		kernel_bias_fp[8*i+3] = curr_input.data.sub_data_3;
		}
	}
...
#pragma HLS PIPELINE
				...
				curr_output.data.sub_data_0 = post_process_unit(output_acc_0,kernel_bias_fp[8*input_ch_idx],bias_en,leaky);
				curr_output.data.sub_data_1 = post_process_unit(output_acc_1,kernel_bias_fp[8*input_ch_idx+1],bias_en,leaky);
				curr_output.data.sub_data_2 = post_process_unit(output_acc_2,kernel_bias_fp[8*input_ch_idx+2],bias_en,leaky);
				curr_output.data.sub_data_3 = post_process_unit(output_acc_3,kernel_bias_fp[8*input_ch_idx+3],bias_en,leaky);
				...
				if((input_ch_idx == MAX_KERNEL_NUM/8-1)
	              &&(col_idx == input_w-1)
				  &&(row_idx == input_h-1))
					curr_output.last = 1;
				else
